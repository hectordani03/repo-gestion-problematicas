...
import { useAccessibility } from '../context/AccesibilityContext.jsx'; // Asegúrate de que la ruta sea correcta
...
export default function Sidebar () {
    ...
    // Obtiene los estados y datos del hook de sidebar (simulados)
    const {
        openProyectos,
        setOpenProyectos,
        proyectos,
  ...
  return (
    // La barra lateral ocupa la altura completa de la pantalla y tiene sombra
    <aside className="sidebar w-2/12 h-screen relative shadow-xl">
        <nav>
            <ul className="flex flex-col gap-4 p-4 items-start text-xl mt-8 ml-5">
                ...
                {/* Enlace a Solicitar un proyecto */}
                <li className="flex items-center gap-4">
                ...
                <li className="flex flex-col items-start w-full">
                    ...
                    >
                        <div className="flex items-center gap-4">
                            <svg className={`text-4xl ${isDark ? 'text-white' : ''}`} xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24"><rect width="10" height="10" x="12" y="2" fill="currentColor" rx="2"/><path fill="currentColor" d="M12 7h-1c-1.886 0-2.828 0-3.414.586S7 9.114 7 11v2c0 1.886 0 2.828.586 3.414S9.114 17 11 17h2c1.886 0 2.828 0 3.414-.586S17 14.886 17 13v-1h-1c-1.886 0-2.828 0-3.414-.586S12 9.886 12 8z" opacity=".7"/><path fill="currentColor" d="M7 12v1c0 1.886 0 2.828.586 3.414S9.114 17 11 17h1v1c0 1.886 0 2.828-.586 3.414S9.886 22 8 22H6c-1.886 0-2.828 0-3.414-.586S2 19.886 2 18v-2c0-1.886 0-2.828.586-3.414S4.114 12 6 12z" opacity=".4"/></svg>
                            Mis proyectos
                        </div>
                        <svg className={`text-2xl transform transition-transform duration-200 ${openProyectos ? 'rotate-180' : 'rotate-0'} ${isDark ? 'text-white' : ''}`} xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24"><path fill="currentColor" d="m12 13.171l4.95-4.95l1.414 1.415L12 16L5.636 9.636L7.05 8.222z"/></svg>
                    </button>
                    {openProyectos && (
                    <ul className={`ml-18 text-base mt-1 space-y-1 w-6/12 ${isDark ? 'text-white' : ''} ${largeText ? 'text-xl' : ''} ${dyslexiaFont ? 'font-dyslexia' : ''}`}>
                ...
                <li className="flex flex-col items-start w-full">
                    ...
                    >
                        <div className="flex items-center gap-4">
                            <svg className={`text-4xl ${isDark ? 'text-white' : ''}`} xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24"><path fill="currentColor" d="M17.562 21.56a1 1 0 0 1-.465-.115L12 18.765l-5.097 2.68a1 1 0 0 1-1.451-1.054l.973-5.676l-4.123-4.02a1 1 0 0 1 .554-1.705l5.699-.828l2.548-5.164a1.042 1.042 0 0 1 1.794 0l2.548 5.164l5.699.828a1 1 0 0 1 .554 1.706l-4.123 4.019l.973 5.676a1 1 0 0 1-.986 1.169"/></svg>
                            Mis favoritos
                        </div>
                        <svg className={`text-2xl transform transition-transform duration-200 ${openFavoritos ? 'rotate-180' : 'rotate-0'} ${isDark ? 'text-white' : ''}`} xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24"><path fill="currentColor" d="m12 13.171l4.95-4.95l1.414 1.415L12 16L5.636 9.636L7.05 8.222z"/></svg>
                    </button>
                    {openFavoritos && (
                    <ul className={`ml-18 text-base mt-1 space-y-1 w-6/12 ${isDark ? 'text-white' : ''} ${largeText ? 'text-xl' : ''} ${dyslexiaFont ? 'font-dyslexia' : ''}`}>
                        <hr className='w-11/12 -translate-x-5 text-gray-400' />
                ...
                <li className="flex flex-col items-start w-full">
                    ...
                    >
                        <div className="flex items-center gap-4">
                            <svg className={`text-4xl ${isDark ? 'text-white' : ''}`} xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24"><path fill="currentColor" fillRule="evenodd" d="M8 4a4 4 0 1 0 0 8a4 4 0 0 0 0-8m-2 9a4 4 0 0 0-4 4v1a2 2 0 0 0 2 2h8a2 2 0 0 0 2-2v-1a4 4 0 0 0-4-4zm7.25-2.095c.478-.86.75-1.85.75-2.905a6 6 0 0 0-.75-2.906a4 4 0 1 1 0 5.811M15.466 20c.34-.588.535-1.271.535-2v-1a5.98 5.98 0 0 0-1.528-4H18a4 4 0 0 1 4 4v1a2 2 0 0 1-2 2z" clipRule="evenodd"/></svg>
                            Miembros
                        </div>
                        <svg className={`text-2xl transform transition-transform duration-200 ${openMiembros ? 'rotate-180' : 'rotate-0'} ${isDark ? 'text-white' : ''}`} xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24"><path fill="currentColor" d="m12 13.171l4.95-4.95l1.414 1.415L12 16L5.636 9.636L7.05 8.222z"/></svg>
                    </button>
                    {openMiembros && (
                    <ul className={`ml-18 text-base mt-1 space-y-1 w-6/12 ${isDark ? 'text-white' : ''} ${largeText ? 'text-xl' : ''} ${dyslexiaFont ? 'font-dyslexia' : ''}`}>
                        <hr className='w-11/12 -translate-x-5 text-gray-400' />
